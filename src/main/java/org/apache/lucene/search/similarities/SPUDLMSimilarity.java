 ...
 * It is provided in spud.searching.LMNormQuery
 */
public class SPUDLMSimilarity extends SPUDLMBaseSimilarity {
	...
	protected void explain(Explanation expl, BasicStats stats, int doc, float freq, float docLen) {
		...
		weightExpl.setValue((float) Math.log(1 + freq / (omega * ((LMStats) stats).getPolyaCollectionProbability())));
		weightExpl.setDescription("term weight");
		expl.addDetail(weightExpl);
		expl.addDetail(new Explanation((float) Math.log(omega / (docLen + omega)), "document norm"));
